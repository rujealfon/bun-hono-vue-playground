...
var notFoundHandler2 = (c) => {
  return c.json(createErrorResponse("Endpoint not found"), HTTP_STATUS.NOT_FOUND);
};
var errorHandler2 = (err, c) => {
  console.error("Server error:", err);
  return c.json(createErrorResponse("Internal server error"), HTTP_STATUS.INTERNAL_SERVER_ERROR);
};
...
tasks.get("/", (c) => {
  try {
    ...
    return c.json(createErrorResponse("Failed to fetch tasks"), HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }
});
tasks.get("/:id", (c) => {
  try {
    ...
    if (!task) {
      return c.json(createErrorResponse("Task not found"), HTTP_STATUS.NOT_FOUND);
    }
    return c.json(createSuccessResponse(sanitizeTask(task)));
  } catch (error) {
    console.error("Error fetching task:", error);
    return c.json(createErrorResponse("Failed to fetch task"), HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }
});
tasks.post("/", async (c) => {
  try {
    ...
    if (validationErrors.length > 0) {
      return c.json(createValidationErrorResponse(validationErrors), HTTP_STATUS.BAD_REQUEST);
    }
    const createTaskData = body;
    ...
    return c.json(createSuccessResponse(sanitizeTask(newTask)), HTTP_STATUS.CREATED);
  } catch (error) {
    console.error("Error creating task:", error);
    return c.json(createErrorResponse("Failed to create task"), HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }
});
tasks.put("/:id", async (c) => {
  try {
    ...
    return c.json(createErrorResponse("Failed to update task"), HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }
});
tasks.delete("/:id", (c) => {
  try {
    ...
    return c.json(createSuccessResponse({ message: "Task deleted successfully" }));
  } catch (error) {
    console.error("Error deleting task:", error);
    return c.json(createErrorResponse("Failed to delete task"), HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }
});
tasks.patch("/:id/toggle", (c) => {
  try {
    ...
    return c.json(createErrorResponse("Failed to toggle task"), HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }
});
tasks.get("/stats", (c) => {
  try {
    const stats = {
      ...
      overdue: tasksList.filter((t) => !t.completed && t.dueDate && new Date(t.dueDate) < new Date).length,
      byPriority: {
    ...
    return c.json(createErrorResponse("Failed to fetch statistics"), HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }
});
tasks.post("/bulk", async (c) => {
  try {
    ...
    if (!action || !Array.isArray(taskIds)) {
      return c.json(createErrorResponse("Invalid bulk operation request"), HTTP_STATUS.BAD_REQUEST);
    }
    let updatedCount = 0;
    switch (action) {
      case "complete":
      ...
      case "delete":
        ...
        break;
      default:
        return c.json(createErrorResponse("Invalid bulk action"), HTTP_STATUS.BAD_REQUEST);
    }
    ...
    return c.json(createErrorResponse("Failed to perform bulk operation"), HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }
});
